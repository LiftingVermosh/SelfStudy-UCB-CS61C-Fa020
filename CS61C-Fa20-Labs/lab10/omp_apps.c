...
double* gen_array(int n) {
  ...
  // return array;
  /* x86 架构下drand48缺失，使用 rand 替代 */
  double* array = (double*)malloc(n * sizeof(double));
  static int seeded = 0;  // 静态变量确保只初始化一次随机种子
  if (!seeded) {
    srand(time(NULL));  // 使用当前时间初始化随机种子
    seeded = 1;
  }
  for (int i = 0; i < n; i++) {
    array[i] = (double)rand() / (RAND_MAX + 1.0);  // 生成[0.0, 1.0)之间的随机双精度数
  }
  return array;
}
...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    ...
    int thd_size = omp_get_num_threads();
    for(int i=tid; i<ARRAY_SIZE; i+=thd_size){
      z[i] = x[i] + y[i];
    }
  }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    int tid = omp_get_thread_num();
    int thd_size = omp_get_num_threads();
    int chunk_size = ARRAY_SIZE / thd_size;
    int start = tid * chunk_size;
    int end = start + chunk_size;
    if(tid == thd_size - 1) end = ARRAY_SIZE;
